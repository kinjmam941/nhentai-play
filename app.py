...
def fetch_videos(url):
    try:
        ...
        results = soup.find_all('div', {'class': 'video-item'})
        videos = []
        for result in results:
            try:
                title = result.find('a', {'class': 'video-title'})['title']
                link = result.find('a', {'class': 'video-title'})['href']
                views = result.find('span', {'class': 'views'}).text.strip()
                thumbnail = result.find('img', {'class': 'thumbnail'})['src'] if result.find('img', {'class': 'thumbnail'}) else None
                duration = result.find('span', {'class': 'duration'}).text.strip() if result.find('span', {'class': 'duration'}) else 'N/A'
                videos.append({
                    'title': title, 
                    'link': link, 
                    'views': views,
...
@app.route('/about')
def about():
    return render_template_string(ABOUT_PAGE, active_page='about')
